  ...
  while (stack.length > 0) {
    ...
    for (let d of directions) {
      ...
      if (nx > 0 && nx < CONFIG.cols - 1 && ny > 0 && ny < CONFIG.rows - 1) {
        if (state.grid[ny] && state.grid[ny][nx] && !state.grid[ny][nx].visited) neighbors.push(state.grid[ny][nx]);
      }
    }
    ...
    if (neighbors.length > 0) {
      ...
      if (state.grid[my] && state.grid[my][mx]) state.grid[my][mx].isWall = false;
  ...
  for (let y = 1; y < CONFIG.rows - 1; y++) {
    for (let x = 1; x < CONFIG.cols - 1; x++) {
      if (state.grid[y] && state.grid[y][x] && state.grid[y][x].isWall && nextRandom() < chance) {
        // Рядом со стартом разрушаемые стены не создаются
        if (x < 3 && y < 3) continue;
...
export function clearVisitedFlags() {
  for (let y = 0; y < CONFIG.rows; y++) {
    for (let x = 0; x < CONFIG.cols; x++) {
      if (state.grid[y] && state.grid[y][x]) state.grid[y][x].visited = false;
    }
  }
}
