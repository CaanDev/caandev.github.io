...
export function generateBossArena() {
  ...
  for (let y = 1; y < arenaSize - 1; y++) {
    for (let x = 1; x < arenaSize - 1; x++) {
      if (state.grid[y] && state.grid[y][x]) state.grid[y][x].isWall = false;
    }
  }
  ...
  if (state.grid[exitY] && state.grid[exitY][exitX]) state.grid[exitY][exitX].isWall = false;
  ...
  if (bossLevel === 15) spawnPillars(arenaSize);
...
function spawnPillars(arenaSize) {
  ...
  for (let i = 0; i < PILLAR_CONFIG.innerCount; i++) {
    ...
    if (px > 1 && px < arenaSize - 2 && py > 1 && py < arenaSize - 2) createPillar(px, py);
  }
  ...
  for (let i = 0; i < PILLAR_CONFIG.outerCount; i++) {
    ...
    if (px > 1 && px < arenaSize - 2 && py > 1 && py < arenaSize - 2) {
      ...
      if (distToCenter > PILLAR_CONFIG.innerRadius + 1.5) createPillar(px, py);
    }
  }
}
...
export function updateBossSpawnAnimation() {
  ...
  if (state.bossSpawnTimer > 0) {
    ...
    if (state.bossSpawnTimer <= 0) spawnBoss();
  }
}
  ...
  state.monsters.push({
    ...
    updatePhase: function(currentHp, maxHp) {
      ...
      for (let i = 0; i < this.phaseThresholds.length; i++) {
        if (percent < this.phaseThresholds[i]) newPhase = i + 2; else break;
      }
