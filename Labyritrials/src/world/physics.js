...
export function findPath(startX, startY, targetX, targetY) {
  ...
  // Проверка расстояния (если слишком далеко - отмена поиска)
  ...
  // Для коротких расстояний используется BFS (быстрее)
  if (dist < 5) {
  ...
  // Для длинных расстояний используется A* (эффективнее)
...
function findPathAStar(start, target) {
  ...
  while (!openSet.isEmpty() && maxSteps-- > 0) {
    ...
    for (const dir of directions) {
      ...
      // Пропуск, если уже в закрытом множестве
      ...
      if (tentativeG < (gScore.get(neighborKey) || Infinity)) {
        ...
        // Добавление в открытое множество только если ещё не там
        let inOpenSet = false;
        ...
        if (!inOpenSet) openSet.push(neighborKey, fScore.get(neighborKey));
      }
    }
  }
  
  // Если путь не найден, пробуется BFS как fallback
