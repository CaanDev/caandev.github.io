...
export function generateTrapPortal() {
  ...
  while (!found && attempts < maxAttempts) {
    ...
    if (state.grid[wallY] && state.grid[wallY][wallX] &&
      ...
      if (state.treasurePortal && state.treasurePortal.x === wallX && state.treasurePortal.y === wallY) alreadyUsed = true;
...
export function generateTrapRoom() {
  ...
  // Сохранение раскрытых клеток (для мини-карты)
...
export function returnFromTrapRoom() {
  ...
  // Сохранение портала входа для деактивации
  ...
  if (state.originalTreasurePortal) state.treasurePortal = state.originalTreasurePortal;
  ...
  // Проверка формата сохранённых артефактов
  if (state.originalArtifacts && state.originalArtifacts.length > 0) {
    if (state.originalArtifacts[0].gridX !== undefined) {
      state.artifacts = state.originalArtifacts.map(a => ({
        ...
        artifactSpriteName: a.artifactSpriteName,
        artifactBiome: a.artifactBiome,
      }));
    } else {
  ...
  if (state.returnPortal) {
    ...
    if (state.grid[targetY] && state.grid[targetY][targetX] && !state.grid[targetY][targetX].isWall) {
      player.x = targetX;
      player.y = targetY;
  ...
  // Проверка, что игрок не стоит на портале
  if (state.trapPortal && 
