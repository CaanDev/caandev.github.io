...
export function createFakeExitPortal() {
  ...
  if (state.grid[centerY] && state.grid[centerY][centerX]) state.grid[centerY][centerX].isFakePortal = true;
}
...
export function showRealExitPortal() {
  if (state.trapExitRevealed) return;
  ...
  if (state.trapFakePortal) {
    state.trapFakePortal.active = false;
    const { x, y } = state.trapFakePortal;
    if (state.grid[y] && state.grid[y][x]) state.grid[y][x].isFakePortal = false;
...
export function spawnMimicHunterChest() {
  // Проверка, есть ли уже талисман у игрока
  if (player.inventory?.items?.equipment?.includes('talismanMimicHunter')) return;
  // Проверка, не появлялся ли уже сундук
  ...
  // Проверка, что центр не занят
  if (state.grid[centerY]?.[centerX]?.isWall) return;
  
  // Создание сундука в центре
  state.chests.push({
