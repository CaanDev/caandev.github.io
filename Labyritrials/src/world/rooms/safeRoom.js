...
export function generateSafeRoom() {
  ...
  if (state.originalHadMap === undefined) state.originalHadMap = player.hasMap;
...
function spawnSafeRoomTorches() {
  ...
  // Правая стена (пропуск выхода на y=4)
  for (let y = 1; y < size - 1; y += 2) {
    if (y === 4) continue;
...
function createSafeRoomChest() {
  ...
  // Спрайт для зелья
  // Определение биома для зелья - безопасная комната
  const potionBiome = 'safe';
  // Получение имени спрайта через getRandomSprite
...
export function returnFromSafeRoom() {
  ...
  if (state.originalHadMap) {
    for (let y = 0; y < CONFIG.rows; y++) {
      for (let x = 0; x < CONFIG.cols; x++) {
        if (state.grid[y] && state.grid[y][x]) state.grid[y][x].revealed = true;
      }
    }
  } else {
