...
import { loadEssentialTemplates, showLoader, updateLoader, hideLoader, clearAllCaches, logger, formatNumber } from './utils/index.js';
...
import { loadAllAtlases, getAtlasStats, playerAnimator } from './sprites/index.js';
    ...
    if (hasSave()) {
      ...
      if (info) {
        saveInfo.innerHTML = `
          ...
          🎭 Уровень: ${formatNumber(info.level)}<br>
          💰 Золото: ${formatNumber(info.gold)}<br>
          ⚔️ Урон: ${formatNumber(info.damage)}<br>
          ❤️ HP: ${formatNumber(info.hp)} / ${formatNumber(info.maxHp)}<br>
  ...
  return new Promise((resolve) => {
    ...
    if (settings.musicEnabled) {
      if (state.inSafeRoom) {
        ...
        if (!audio.music.isPlaying) audio.music.play('safeRoom');
      } else {
...
function setupStartScreenButtons() {
  ...
  // Кнопка "Загрузить"
  if (loadSaveBtn) setupLoadSaveHandler(loadSaveBtn);
    ...
    newNewGameBtn.addEventListener('click', async () => {
      ...
      if (await checkExistingSave()) {
        const confirmed = await showConfirm(
          ...
          { title: 'Новая игра', yesText: 'Начать', noText: 'Отмена' }
        );
        
        if (!confirmed) return;
      }
      
      // Отображение окна выбора персонажа
...
async function init() {
  ...
  // Загрузка критических HTML-шаблонов
  ...
  updateLoader('Загрузка спрайт-атласов...', '🖼️', 'Загрузка текстур...', 50);
  await loadAllAtlases((progress) => {
    const mappedProgress = 70 + (progress * 0.15); // 70-85%
    updateLoader(null, null, null, Math.min(85, mappedProgress));
  });

  // Загрузка спрайтов
  updateLoader('Загрузка спрайтов...', '🖼️', 'Подготовка персонажа...', 60);
  ...
  // Установка пола персонажа (по умолчанию мужской)
  playerAnimator.setGender(player.gender || 'male');

  // Загрузка спрайтов достижений
  updateLoader('Загрузка спрайтов достижений...', '🏆', 'Подготовка иконок...', 68);
  
  const achievementSprites = await import('./sprites/achievementSprites.js');
  await achievementSprites.preloadAchievementSprites((progress) => {
    const mappedProgress = 68 + (progress * 0.02);
    updateLoader(null, null, null, Math.min(70, mappedProgress));
  });
  ...
  if (settings.musicEnabled) audio.forcePlayMusic('menu'); else audio.setMusicMode('menu');
  ...
  updateLoader('Создание мира...', '🗺️', 'Генерация лабиринта...', 80);
  
  await new Promise(resolve => setTimeout(resolve, 300));

  // Принудительная установка биома
  state.currentBiome = 'cave';
  ...
  // Добавление обработчика клавиатуры для инвентаря
