...
export function handleMonsterDeath(m, index, monstersArray) {
  ...
  // Остановка звука эффекта шока (если есть)
  if (m._shockSound) {
    audio.sound.stopEffectSound(m._shockSound);
    m._shockSound = null;
  }

  // Сброс флага "Тень" (достижение — пройти уровень без убийств)
  state.shadowActive = false;
  
  // Обновление статистики и достижений
  ...
  if (m.isTrapMonster) {
    ...
    if (trapIndex !== -1) state.trapMonsters.splice(trapIndex, 1);
    state.trapMonstersKilled = (state.trapMonstersKilled || 0) + 1;

    // Проверка завершения волны в комнате-ловушке
  ...
  if (canDrop) handleMonsterDrop(m);
  // Эффект вампиризма игрока (посох вампира)
  if (player.meleeWeapon === 'vampire') spawnBloodDrops(m.x, m.y, true);

  // Удаление монстра из массива
  monstersArray.splice(index, 1);
}
