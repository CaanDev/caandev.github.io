...
export function updateTrapGlowTimer(m) {
  ...
  if (m.stunTimer > 0 && m.trapGlowColor === COLORS.monsters.trapGlow.stun) {
    m.trapGlowTimer = m.stunTimer; // Синхронизация таймеров
    return;
  }
  ...
  if (m.trapGlowTimer > 0) {
    m.trapGlowTimer--;
    if (m.trapGlowTimer <= 0) m.trapGlowColor = null;
  }
}
...
export function updateShockEffect(m, i) {
  ...
  // Если шок закончился - остановка звука
  if (m.shockTimer <= 0) {
    if (m._shockSound) {
      audio.stopEffectSound(m._shockSound);
      m._shockSound = null;
  ...
  // Уменьшение таймера и увеличение тика
  m.shockTimer--;
...
export function updateGhostGlow(m) {
  ...
  if (m.state === 'chase') m.glowIntensity = Math.min(1, (m.glowIntensity || 0) + 0.05);
  else m.glowIntensity = Math.max(0, (m.glowIntensity || 0) - 0.05);
}