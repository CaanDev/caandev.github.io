...
export function updateBossLogic(m, i) {
  ...
  if (m.bossType === BOSS_TYPES.DEMON && m.isPreparingBeam) {
    if (m.abilities.tremor && m.abilities.tremor.tryExecuteBeam) m.abilities.tremor.tryExecuteBeam(m);
  }

  // Обновление фазы
  const phaseChanged = m.updatePhase(m.hp, m.maxHp);

  // Демон: изменение поведения при смене фазы
  if (m.bossType === BOSS_TYPES.DEMON && phaseChanged) {
    if (m.currentPhase === 2) {
      // Фаза 2: переключение в режим избегания, призыв миньонов + ускорение
      m.state = 'flee';
    ...
    if (m.currentPhase === 3) {
      // Фаза 3: переключение в режим избегания, ярость + усиленный призыв
      m.state = 'flee';
  ...
  if (m.bossType === BOSS_TYPES.DEMON && (m.currentPhase === 2 || m.currentPhase === 3)) {
    // Если босс готовит луч — пропуск призыва
    if (m.isPreparingBeam) return abilitiesUsed;
  ...
  if (m.bossType === BOSS_TYPES.MIND) {
    ...
    if (currentPhase !== m.lastPhase) {
      ...
      if (currentPhase === 2) m.phase2MessageShown = false;
      if (currentPhase === 3) {
  ...
  if (m.bossType === BOSS_TYPES.DUO_SHOOTER) {
    ...
    if (!chaserAlive && !m.rageModeActive) {
      ...
      if (m.originalSpeed === undefined) m.originalSpeed = m.speed;
