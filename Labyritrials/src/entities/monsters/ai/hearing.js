...
export function addSoundEvent(x, y, type, radius) {
  if (!state.soundEvents) state.soundEvents = [];

  // Если радиус не указан — используется базовый
...
function markPlaceAsChecked(m, x, y) {
  if (!m._checkedPlaces) m._checkedPlaces = [];
  const key = `${Math.round(x)},${Math.round(y)}`;
  if (!m._checkedPlaces.includes(key)) m._checkedPlaces.push(key);
  if (m._checkedPlaces.length > 20) m._checkedPlaces.shift();
}
...
function findClosestWalkableCell(targetX, targetY, maxRadius = 3) {
  ...
  if (centerCell && !centerCell.isWall && !centerCell.isPillar && centerCell.revealed) return { x: targetX, y: targetY };
  ...
  if (bestDist < maxRadius * cellSize) return { x: bestX, y: bestY };
...
function markSoundAsHeard(m, sound) {
  ...
  if (!sound.heardBy.includes(monsterId)) sound.heardBy.push(monsterId);
}
...
function clearHeardSounds(m) {
  ...
  for (const sound of state.soundEvents) {
    if (sound.heardBy && sound.heardBy.includes(monsterId)) {
      ...
      if (index !== -1) sound.heardBy.splice(index, 1);
    }
  }
}
...
function clearSoundMemory(m) {
  m._checkedPlaces = [];
  clearHeardSounds(m);
  if (m._isReactingToSound) resetSoundReaction(m);
}
...
function getReactionDuration(distToSound, gameLevel) {
  ...
  if (gameLevel >= 10) duration = Math.floor(duration * 1.5);
...
function isMonsterStuck(m) {
  ...
  if (moved < 1) m._stuckMoveCounter = (m._stuckMoveCounter || 0) + 1;
  else m._stuckMoveCounter = 0;
...
export function updateMonsterHearing(m, distToPlayer, hasLineOfSight) {
  ...
  if (hasLineOfSight && distToPlayer < m.vision) {
    // Если монстр отреагировал на звук — сброс
    if (m._isReactingToSound) clearSoundMemory(m);
    // Если монстр в режиме поиска — переключение в преследование
    if (m.isSearching) {
      m.isSearching = false;
  ...
  if (isActivelyChasing) {
    // Если монстр отреагировал на звук — сброс
    if (m._isReactingToSound) clearSoundMemory(m);
    // Продолжение преследования — возвращение null (не реагирует на звуки)
    return null;
  }

  // Если монстр уже реагирует на звук — продолжение реакции
  if (m._isReactingToSound) return updateSoundReaction(m);
...
function getNearestSoundWithRadius(m, hearingRadius) {
  ...
  for (const sound of state.soundEvents) {
    ...
    // Используется радиус слуха монстра (или звука, если он больше)
...
function updateSoundReaction(m) {
  if (m._reactionTimer > 0) m._reactionTimer--;
  ...
  if (m._reactionPhase === 'move') {
    ...
    if (isMonsterStuck(m) && distToTarget > 50) return forceFinishReaction(m, 'stuck');
    if (m._reactionTimer <= 0 && distToTarget > 50) return forceFinishReaction(m, 'timeout');
    ...
    if (m._reactionTimer <= 0) {
      if (distToTarget < 100) m._reactionTimer = 30;
      else return forceFinishReaction(m, 'timeout_far');
    }
