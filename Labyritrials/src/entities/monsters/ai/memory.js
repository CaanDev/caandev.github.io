...
export function updateMonsterMemory(m, distToPlayer, hasLineOfSight) {
  // Если монстр реагирует на звук - пропуск логики памяти
  if (m._isReactingToSound) return { isSearching: false };

  // Инициализация
  initMovementState(m);

  // Увеличение длительности памяти
  // Если у монстра ещё не установлена длительность памяти - установка
  if (m.memoryDuration === undefined || m.memoryDuration < MEMORY_DURATION) m.memoryDuration = MEMORY_DURATION;
  
  // Установка длительности усиленной видимости
  if (m.visionBoostDuration === undefined || m.visionBoostDuration < VISION_BOOST_DURATION) m.visionBoostDuration = VISION_BOOST_DURATION;
  if (m.visionBoostMultiplier === undefined) m.visionBoostMultiplier = VISION_BOOST_MULTIPLIER;

  // Если есть видимость - обновление памяти
  ...
  // Если видимости нет, но есть память - поиск
  if (m.lastKnownX !== null && m.memoryTimer > 0) {
    if (m.visionBoosted) {
      m.visionBoostTimer--;
      if (m.visionBoostTimer <= 0) deactivateVisionBoost(m);
    }
    ...
    if (m.predictionTimer > 0) m.predictionTimer--;
    ...
    if (usingPrediction) {
      ...
      if (m.searchTimer > 500) return transitionToPatrol(m, 'долгого поиска');

      // Расширенный поиск
      if (m.searchTimer > 300) return handleExpandedSearch(m, searchProgress, distToMemory);
