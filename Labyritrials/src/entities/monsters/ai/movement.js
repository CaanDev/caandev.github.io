...
function isMonsterStuckInCorner(m) {
  // Проверка, не двигался ли монстр последние 30 кадров
  ...
  if (moved < 1) m._stuckCounter++; else m._stuckCounter = 0;
  
  // Если стоит на месте больше 60 кадров — застрял
  if (m._stuckCounter > 60) return true;
...
function escapeFromCorner(m) {
  // Сброс счётчика застревания
  m._stuckCounter = 0;
  
  // Переключение на следующую точку маршрута (пропуск текущей)
  m.patrolIndex = (m.patrolIndex + 1) % m.patrolPath.length;
  
  // Генерирование нового маршрута, если точек мало
  ...
  // Сброс таймера, чтобы монстр продолжил движение
  m.patrolTimer = 0;
...
export function updateChaseMovement(m) {
  // Если игрок отключен - нет движения
...
export function updatePatrolMovement(m) {
  ...
  // Проверка не чаще чем раз в 90 кадров
  if (m.patrolTimer <= 0 && m.patrolPhase === 'moving') {
    ...
    // Фильтрация уже проверенных точек
    ...
    if (availablePoints.length > 0 && Math.random() * 100 < m.curiosity * 0.3) {
      // Выбор точки с самым высоким приоритетом
  ...
  if (m.patrolPhase === 'investigating') {
    ...
    if (finished) m.patrolTimer = 30 + Math.random() * 60;
    return;
  }
  ...
  if (m.patrolPhase === 'moving') {
    // Обновление таймера
    if (m.patrolTimer > 0) m.patrolTimer--;
    ...
    if (!target) {
      // Если маршрут пуст — повторная генерация
      m.patrolPath = generatePatrolPath(m, 4 + Math.floor(Math.random() * 2), 8 + Math.floor(Math.random() * 4));
      m.patrolIndex = 0;
      return;
    }
    ...
    if (distToTarget < 30) {
      // Достигнута точка — переход к следующей
      m.patrolIndex = (m.patrolIndex + 1) % m.patrolPath.length;
      m.patrolTimer = 20 + Math.random() * 40;
    } else {
...
export function generatePatrolPath(m, pointCount = 4, maxRadius = 8) {
  ...
  while (path.length < pointCount && attempts < maxAttempts) {
    ...
    // Подсчёт количества проходимых соседей
    ...
    for (const [ndx, ndy] of neighborDirs) {
      ...
      if (nx >= 0 && nx < CONFIG.cols && ny >= 0 && ny < CONFIG.rows) {
        ...
        if (neighbor && !neighbor.isWall) walkableNeighbors++;
      }
    }
    
    // Если у точки меньше 2 проходимых соседей — это тупик, пропуск
  ...
  // Если не найдено достаточно точек — добавление простых (с проверкой на тупики)
  if (path.length < 3) {
...
export function findInterestingPoints(m, radius = 350) {
  ...
  if (CONFIG.goal && CONFIG.goal.x > 0 && CONFIG.goal.y > 0) {
    ...
    if (dist < radius) {
      // Проверка, доступен ли выход (не босс-уровень или босс побеждён)
  ...
  // Проверка клеток вокруг монстра
  ...
  for (let dy = -checkRadius; dy <= checkRadius; dy++) {
    for (let dx = -checkRadius; dx <= checkRadius; dx++) {
      ...
      // Поиск разрушенных стен (isWall === false, но была стеной)
      // Используется специальный флаг wasBreakable
      if (cell.wasBreakable && !cell.isWall) {
        ...
        if (dist < radius) {
          // Проверка, не проверяли ли уже эту стену
  ...
  // Возвращаются только уникальные точки (не слишком близко друг к другу)
  const uniquePoints = [];
  const minDist = 80;
...
export function startInvestigation(m, point) {
  ...
  // Ограничение списка проверенных точек
  if (m.checkedInterests.length > 20) m.checkedInterests.splice(0, m.checkedInterests.length - 20);
  
  // Визуальный эффект — вопросительный знак
