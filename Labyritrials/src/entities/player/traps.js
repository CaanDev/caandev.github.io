...
export function checkTraps() {
  for (let t of state.traps) {
    ...
    if (!t.triggered && t.resetTimer <= 0) {
      if (Math.hypot(player.px - t.x, player.py - t.y) < 38) activateTrap(t);
    }
  }
}
...
function activateTrap(t) {
  ...
  if (didEvade) handleEvasion(evasionChance, t);
  else applyTrapEffectToPlayer(t, evasionChance);
}
...
function handleEvasion(evasionChance, t) {
  ...
  // Определение направления уворота
  if (isMoving) {
    evadeX = player.dirX;
    evadeY = player.dirY;
  } else {
...
function applyTrapEffectToPlayer(t, evasionChance) {
  ...
  switch (t.type) {
    ...
    case 'lightning': {
      ...
      // Остановка предыдущего звука эффекта
    ...
    default: {
      ...
      if (player.incomingDamageMultiplier) damageTaken = Math.floor(damageTaken * player.incomingDamageMultiplier);
      playerTakeDamageFromMonster(damageTaken);
