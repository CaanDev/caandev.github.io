...
import { getBloodTrails } from '../player/bloodTrails.js';
...
function getWeaponColor() {
  ...
  if (player.meleeWeapon === 'vampire') color = TRAIL_CONFIG.colors.vampire;
  else if (player.meleeWeapon === 'stun') color = TRAIL_CONFIG.colors.stun;
  return color;
}
...
/**
 * Проверка, стоит ли игрок на кровавой луже от монстра
 * 
 * @returns {boolean} - true, если игрок на крови монстра
 * @private
 */
function isPlayerOnMonsterBlood() {
...
/**
 * Проверка, стоит ли игрок на своих кровавых следах
 * 
 * @returns {boolean} - true, если игрок на своей крови
 * @private
 */
function isPlayerOnOwnBloodTrails() {
  const bloodTrails = getBloodTrails();
  if (!bloodTrails || bloodTrails.length === 0) return false;
  
  const checkRadius = 25;
  
  for (const trail of bloodTrails) {
    const dist = Math.hypot(player.px - trail.x, player.py - trail.y);
    if (dist < checkRadius + trail.size * 0.5) return true;
  }
  return false;
}

/**
 * Проверка, стоит ли игрок на любой крови (своей или монстра)
 * 
 * @returns {boolean} - true, если игрок на крови
 * @private
 */
function isPlayerOnAnyBlood() {
  return isPlayerOnMonsterBlood() || isPlayerOnOwnBloodTrails();
}
...
function updateBloodTrailState() {
  const now = Date.now();
  const onBlood = isPlayerOnAnyBlood();
...
export function updatePlayerTrails() {
  for (let i = state.playerTrails.length - 1; i >= 0; i--) {
    ...
    if (trail.life <= 0) state.playerTrails.splice(i, 1);
  }
}
