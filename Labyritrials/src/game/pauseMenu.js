...
import { logger, formatNumber, exitToMainMenu,
         loadTemplateIfNeeded, isTemplateLoaded, isTemplateInitialized, initTemplateHandlers } from '../utils/index.js';
...
function updatePauseStats() {
  ...
  document.getElementById('pause-hp').textContent = formatNumber(Math.floor(player.hp));
  document.getElementById('pause-maxhp').textContent = formatNumber(player.maxHp);
  document.getElementById('pause-gold').textContent = formatNumber(player.gold);
  document.getElementById('pause-damage').textContent = formatNumber(player.baseDamage);
 ...
 * @private
 */
async function handleExitToMainMenu() {
  // Закрытие паузы
  const pauseMenu = document.getElementById('pause-menu');
  ...
  // Запуск игрового цикла (чтобы UI обновился)
  Game.startLoop();
  
  audio.isGameActive = true;
  audio.resume();
  
  // Выход в главное меню (с confirm)
