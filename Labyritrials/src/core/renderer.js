...
import { drawBloodTrails } from '../entities/player/bloodTrails.js';
...
export const Renderer = {
   ...
   */
  draw(ctx, canvas) {
    // Если игрок мёртв — заливка чёрным фоном
    if (player.hp <= 0) {
    ...
    // Обновление кэша размеров холста
    ...
    // Если это босс-уровень, используется размер арены
    ...
    // Вычисление видимого диапазона клеток
    ...
    // 5. Динамические объекты: поверх всего
    ctx.save();
    ctx.translate(camX, camY);
    ...
    // Предметы, сундуки, мухи
    drawLoot(ctx);
    drawChests(ctx);
    drawFlies(ctx);
    
    // Светлячки: задний слой
    drawFireflies(ctx, 'background');
    ...
    // Игрок
    drawPlayer(ctx);
    // Светлячки: передний слой
    drawFireflies(ctx, 'foreground');
    ...
    if (state.isBossLevel && state.bossSpawnTriggered && !state.bossSpawned) drawBossSpawnEffect(ctx, canvas);
    ...
    // Ударная волна
    if (state.shockwave) drawShockwave(ctx, camX, camY);
  }
};