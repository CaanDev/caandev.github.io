...
export function drawAdaptationIndicator(ctx, canvas) {
  ...
  if (state.monsterAdaptation.fireImmunity) adaptations.push({ icon: '🔥', name: 'Огнеупорность', color: COLORS.effects.fire, desc: 'Иммунитет к огню' });
  if (state.monsterAdaptation.stunImmunity) adaptations.push({ icon: '⚡', name: 'Стойкость', color: COLORS.effects.lightning, desc: 'Иммунитет к оглушению' });
  if (state.monsterAdaptation.healingBlock) adaptations.push({ icon: '🧛', name: 'Блок лечения', color: COLORS.effects.vampire, desc: 'Лечение игрока -50%' });
  if (state.monsterAdaptation.healthBoost) adaptations.push({ icon: '🛡️', name: 'Закалка', color: COLORS.effects.stun, desc: '+50% к HP монстров' });
  ...
  // Текст заголовка
  ctx.font = 'bold 11px "Comfortaa", Arial';
