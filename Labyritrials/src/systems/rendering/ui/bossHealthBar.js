...
export function drawBossHealthBar(ctx) {
  ...
  if (Math.abs(diff) > MIN_HP_CHANGE) state.bossDisplayHp[bossId] += diff * SMOOTH_SPEED;
  else state.bossDisplayHp[bossId] = targetHp;
  ...
  // Имя босса
  ctx.font = 'bold 15px "Comfortaa", "Courier New", monospace';
  ...
  // Текст HP
  ctx.font = 'bold 13px "Comfortaa", "Courier New", monospace';
  ctx.fillStyle = textColor;
...
function drawDuoBossHealthBar(ctx) {
  ...
  if (chaser) {
    ...
    if (Math.abs(diffChaser) > MIN_HP_CHANGE) state.bossDisplayHp[chaserId] += diffChaser * SMOOTH_SPEED;
    else state.bossDisplayHp[chaserId] = chaser.hp;
  }
  ...
  if (shooter) {
    ...
    if (Math.abs(diffShooter) > MIN_HP_CHANGE) state.bossDisplayHp[shooterId] += diffShooter * SMOOTH_SPEED;
    else state.bossDisplayHp[shooterId] = shooter.hp;
  }
