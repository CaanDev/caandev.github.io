...
import { CONFIG, state, player, getCellBasedSize } from '../../core/config/index.js';
import { COLORS } from '../../core/config/colors.js';
import { EMOJIS } from '../../emojis.js';
import { getDistanceVisibility } from '../fog/index.js';
import { getStairSpriteName, getSpriteWithSize } from '../../sprites/index.js'; 
...
function drawPortalGate(ctx, portal, baseColor, emoji, customOptions = null) {
  ...
  for (let i = 0; i <= 20; i++) {
    const t = i / 20;
    let brightness;
    if (t < 0.15) brightness = 1.0 - t * 2; else brightness = Math.pow(1 - (t - 0.15) / 0.85, 1.8) * 0.5;
    ...
    if (state.grid[CONFIG.goal.y] && state.grid[CONFIG.goal.y][CONFIG.goal.x]) {
      if (state.grid[CONFIG.goal.y][CONFIG.goal.x].revealed || player.hasMap) {
        ...
        if (canShowPortal) {
          ...
          if (visibility > 0.05) {
            // Определение биома для ступенек
            let biome;
            if (state.isBossLevel) {
            ...
            // Проверка, не обрушены ли ступеньки
            const cell = state.grid[CONFIG.goal.y]?.[CONFIG.goal.x];
            const isCollapsed = cell?.stairsCollapsed === true;
            
            // Выбор группы спрайтов
            const group = isCollapsed ? 'collapsedStairs' : 'stairs';
            const seed = state.gameLevel * 100 + (state.seed || 0);
            const spriteName = getStairSpriteName(biome, seed, group);
            const stairSize = getCellBasedSize('stairs', CONFIG.cellSize);
            const sprite = spriteName ? getSpriteWithSize(spriteName, stairSize) : null;
            
            if (sprite) {
              // Вычисление позиции для центрирования в клетке
              const offsetX = (CONFIG.cellSize - sprite.drawW) / 2;
              const offsetY = (CONFIG.cellSize - sprite.drawH) / 2;
...
function drawSafeRoomPortal(ctx) {
  ...
  if (!isBossLevel && !state.inSafeRoom && state.safePortal && state.safePortal.active && !state.safePortal.hidden) {
    ...
    if (cell && cell.isPortal) drawPortalGate(ctx, state.safePortal, COLORS.safeRoom.portalColor, '');
  }
}
