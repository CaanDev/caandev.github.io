...
import { playerAnimator, PLAYER_DISPLAY_SIZE } from '../../../sprites/index.js';
...
export function drawShopPrompt(ctx) {
  ...
  const fontSize = 16;
  ctx.font = `bold ${fontSize}px "Comfortaa", "Courier New", monospace`;
...
export function drawPlayer(ctx) {
  ...
  if (state.playerVisible === false) {
    // Рисуются только эффекты (если нужно) или ничего
    // Эффекты под спрайтом
    ctx.save();
    drawShockGlow(ctx);
    drawPoisonGlow(ctx);
    drawFreezeGlow(ctx);
  ...
  // Фоновое свечение от шока
  drawShockGlow(ctx);
  // Фоновое свечение от яда
  drawPoisonGlow(ctx);
  // Фоновое свечение от заморозки
  drawFreezeGlow(ctx);
  ...
  // Определение свечения для спрайта
  ...
  if (isNearBookshelf()) {
    ...
    ctx.shadowBlur = 8;
    ctx.shadowColor = 'rgba(0, 0, 0, 0.7)';
    ctx.font = `bold 16px "Comfortaa", "Courier New", monospace`;
    ctx.textAlign = 'center';
    ctx.textBaseline = 'middle';
