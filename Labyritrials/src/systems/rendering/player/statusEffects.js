...
export function getPlayerGlowColor() {
  ...
  if (player.isCharging && player.chargeTime > 30 && 
    ...
    if (player.meleeWeapon === 'vampire') return COLORS.player.glow.vampire;
    else if (player.meleeWeapon === 'stun') return COLORS.player.glow.stun;
...
export function getGlowSize() {
  ...
  // Шок
  if (player.shockTimer > 0) return 55 + Math.sin(Date.now() * 0.025) * 15;
  // Яд
  if (player.poisonTimer > 0 && player.shockTimer <= 0) return 50 + Math.sin(Date.now() * 0.004) * 10;
  // Заморозка
  if (player.isFrozen && player.freezeTimer > 0 && player.shockTimer <= 0 && player.poisonTimer <= 0) return 55 + Math.sin(Date.now() * 0.004) * 10;
