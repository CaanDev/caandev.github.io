...
export function getVisibleCellRange(camX, camY, canvasWidth, canvasHeight, cellSize, cols, rows, padding = 2) {
  // Рендер на 6 клеток дальше, чем нужно
  const renderPadding = 20;

  // Преобразование координат камеры в мировые
  const minWorldX = -camX - renderPadding * cellSize;
  const maxWorldX = -camX + canvasWidth + renderPadding * cellSize;
  const minWorldY = -camY - renderPadding * cellSize;
  const maxWorldY = -camY + canvasHeight + renderPadding * cellSize;
