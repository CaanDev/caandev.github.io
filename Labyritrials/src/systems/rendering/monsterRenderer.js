...
export function drawMonsters(ctx, camX, camY, canvas) {
  ...
  for (let m of state.monsters) {
    ...
    if (!isVisibleWithRadius(m.x, m.y, m.radius + 30, camX, camY, canvas.width, canvas.height)) continue;
    ...
    // На босс-уровнях все монстры видны всегда
    let visibility = 1.0;
    let hasLineOfSightToMonster = true;
    let isInDark = false;
    
    if (!isBossLevel) {
      // Видимость в тумане
      visibility = getDistanceVisibility(m.x, m.y);
      if (visibility <= 0.05) continue;
    
      hasLineOfSightToMonster = hasLineOfSight(player.px, player.py, m.x, m.y);
      const isNearTorch = isNearTorchLight(m.x, m.y);
      isInDark = (!hasLineOfSightToMonster || distToPlayer > CONFIG.glowingEyes.minDistance) && !isNearTorch;
    }
    ...
    if ((m.isBoss || m.isDuoBoss) && m.currentPhase > 1) drawBossPhaseGlow(ctx, m);
    ...
    if (isInDark) {
      ...
      ctx.font = `${m.radius * 1.6}px "Comfortaa", Arial`;
    ...
    if ((m.isBoss || m.isDuoBoss) && !isInDark && visibility > 0.5) {
      ...
      if (m.currentPhase === 2) exclamationText = EMOJIS.ui.exclamation;
      else if (m.currentPhase === 3) exclamationText = EMOJIS.ui.doubleExclamation;
      
      if (exclamationText) {
        ctx.fillStyle = COLORS.ui.textRed;
        ctx.font = `bold ${24 + (m.currentPhase === 3 ? 4 : 0)}px "Comfortaa", Arial`;
        ctx.textAlign = 'center';
    ...
    if (m.isPreparingBeam && m.beamPrepareStart) drawBeamIndicator(ctx, m);
    
    // Полоска здоровья для обычных монстров
    // Используется baseRadius для консистентной ширины полоски
...
function drawGlowingEyes(ctx, m, distToPlayer, visibility) {
  // У боссов нет светящихся глаз в темноте
  if (m.isBoss || m.isDuoBoss) return;
  ...
  if (m.isGhost) eyeColor = COLORS.monsters.eyes.ghost;
  
  // Интенсивность свечения зависит от расстояния и видимости
  let intensity = 0.9 * visibility;
  if (distToPlayer > 400) intensity = 0.6 * visibility;
  else if (distToPlayer < 200) intensity = 1.1 * visibility;
...
function drawBeamIndicator(ctx, boss) {
  ...
  // Текст времени
  ctx.font = 'bold 12px "Comfortaa", "Courier New", monospace';
  ...
  ctx.font = '10px "Comfortaa", "Courier New", monospace';
