...
export function drawBossSummonCircle(ctx) {
  ...
  // Отрисовка круга, пока он виден
  if (opacity > 0.01) drawSummonCircle(ctx, centerX, centerY, radius, time, opacity);
  // Отрисовка частиц (всегда)
...
export function triggerBossSummonFade() {
  ...
  // Создание всплеска частиц при исчезновении
  for (let i = 0; i < 50; i++) {
  ...
  // Добавление мелких частиц для эффекта
  for (let i = 0; i < 30; i++) {
    const angle = Math.random() * Math.PI * 2;
