...
export function startSnowfall() {
  // Проверка на биом Ice
  if (!isIceBiome()) return;
  // Проверка, не в тайной комнате ли игрок
  ...
  // Принудительное создание ледяной маски
  createIceOverlayIfNeeded();

  // Сброс прогресса заморозки при начале снегопада
  ...
  if (snowState.stopTimer) clearTimeout(snowState.stopTimer);
...
function createIceOverlayIfNeeded() {
  ...
  if (!iceOverlay) {
    ...
    // Проверка, что родитель имеет относительное позиционирование
...
export function updateSnowfall() {
  ...
  if (!iceBiome || isInSecretRoom()) {
    if (snowState.active) stopSnowfall();
    return;
  }

  // Если снегопад активен - обновляется прозрачность и выход
  if (snowState.active) {
    updateSnowOpacity();
    return;
  }

  // Проверка, прошла ли минимум 1 минута от начала уровня
  if (!isLevelReadyForSnow()) return;

  // Проверка, прошло ли минимум 3 минуты с последнего снегопада
  const now = Date.now();
  const timeSinceLastEnd = now - snowState.lastSnowfallEnd;
  
  if (timeSinceLastEnd < snowState.minPauseBetween) return;

  // Запуск снегопада с шансом 15% каждую минуту
  if (Math.random() < 0.15) startSnowfall();
}
...
export function initSnowManager() {
  ...
  // Удаление ледяной маски при инициализации
  const iceOverlay = document.getElementById('hp-ice-overlay');
  if (iceOverlay && iceOverlay.parentNode) iceOverlay.parentNode.removeChild(iceOverlay);
  ...
  if (snowState.checkTimer) clearInterval(snowState.checkTimer);
