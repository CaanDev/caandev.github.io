...
class Snowflake {
  ...
  update() {
    ...
    if (this.y > player.py + SNOW_AREA_RADIUS / 2) this.reset();
    
    const dx = this.x - player.px;
    const dy = this.y - player.py;
    const dist = Math.hypot(dx, dy);
    
    if (dist > SNOW_AREA_RADIUS) this.reset();
  }
  ...
  draw(ctx, camX, camY) {
    ...
    // Умножение прозрачности снежинки на общую прозрачность снегопада
...
export function drawSnow(ctx, canvas, camX, camY) {
  // Если снег не активен или полностью прозрачен - не рисуется
  ...
  // Если снежинок нет - создаются
  if (snowflakes.length === 0) createSnowflakes();
  
  // Создание недостающих снежинок
  while (snowflakes.length < MAX_SNOWFLAKES) {
    snowflakes.push(new Snowflake());
  }
