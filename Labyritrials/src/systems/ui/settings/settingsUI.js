...
function showSettings() {
  ...
  if (!audio.isMainMenu) audio.pause();
}
...
export function closeSettings() {
  ...
  if (!audio.isMainMenu && settings.musicEnabled) audio.resume();
...
export function initSettings() {
  ...
  if (settings.showFps) updateFpsVisibility();
  ...
  if (document.querySelector('.settings-tab')) initSettingsHandlers();
  ...
  if (introScreen) observer.observe(introScreen, { attributes: true, attributeFilter: ['style'] });
  if (startScreen) observer.observe(startScreen, { attributes: true, attributeFilter: ['style'] });
  if (gameUI) observer.observe(gameUI, { attributes: true, attributeFilter: ['style'] });
  if (pauseMenu) observer.observe(pauseMenu, { attributes: true, attributeFilter: ['style'] });
}
...
function initTabsScrolling() {
  ...
  // 4. Прокрутка касанием (свайп по вкладкам)
  let touchStartX = 0;
  let touchCurrentX = 0;
