  ...
  try {
    ...
    setTimeout(async () => {
      const confirmed = await showConfirm(
        'Все данные успешно очищены!\n\nДля полного обновления игры будет произведена перезагрузка.',
        { title: '✅ Данные очищены', yesText: 'Перезагрузить', noText: 'Позже' }
      );
...
export async function importSaveData(file) {
  ...
  reader.onload = (event) => {
    try {
      ...
      setTimeout(async () => {
        const confirmed = await showConfirm(
          'Перезагрузить для применения сохранения?',
          { title: '🔄 Перезагрузка', yesText: 'Перезагрузить', noText: 'Позже' }
        );
