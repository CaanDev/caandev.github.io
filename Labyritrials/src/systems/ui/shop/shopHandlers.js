  ...
  if (btn) {
    btn.addEventListener('click', () => {
      if (player.gold >= player.hpCost) {
        ...
        // Обновление UI
        // Сначала вызывается колбэк
        updateShopUICallback();
        
        // Затем обновляются элементы напрямую (с задержкой, чтобы DOM успел обновиться)
        setTimeout(() => {
...
export function initBuyVampireStaffHandler(updateShopUICallback) {
  ...
  if (btn) {
    btn.addEventListener('click', () => {
      ...
      // Если уже куплено - просто показывается сообщение
      if (isOwned) {
  ...
  if (btn) {
    btn.addEventListener('click', () => {
      ...
      if (!isOwned && player.gold >= cost) {
        ...
        // Добавление талисмана в снаряжение
