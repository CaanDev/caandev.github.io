...
function updateWeaponDisplay() {
  ...
  // Ближнее оружие
  const weaponMap = {
    'default': { spriteName: 'invStaffDefault', label: 'Обычный посох' },
    'stun': { spriteName: 'invStaffThunder', label: 'Громовой посох' },
    'vampire': { spriteName: 'invStaffVampire', label: 'Посох вампира' },
  };
  ...
  // Получение спрайта для ближнего оружия
  import('../../sprites/index.js').then(({ getSpriteWithSize }) => {
    const sprite = getSpriteWithSize(melee.spriteName, 40);
    if (meleeImg && sprite) {
      meleeImg.src = sprite.texture.src;
      meleeImg.style.width = `${sprite.drawW}px`;
      meleeImg.style.height = `${sprite.drawH}px`;
      meleeImg.style.objectFit = 'contain';
    }
  });
  ...
  if (rangedContainer) {
    ...
    if (rangedContainer.style.display !== shouldShow) rangedContainer.style.display = shouldShow;
    if (hasRanged) rangedContainer.classList.add('active');
    else rangedContainer.classList.remove('active');
  }
  ...
  if (hasRanged) {
    ...
    // Получение спрайта для огненного шара
    import('../../sprites/index.js').then(({ getSpriteWithSize }) => {
      const sprite = getSpriteWithSize('invFireball', 40);
      if (rangedImg && sprite) {
        rangedImg.src = sprite.texture.src;
        rangedImg.style.width = `${sprite.drawW}px`;
        rangedImg.style.height = `${sprite.drawH}px`;
        rangedImg.style.objectFit = 'contain';
      }
    });
  }
}
...
function shouldUpdateShop() {
  // Если магазин закрыт — сброс кэша и выход
  if (!state.isShopOpen) {
    lastGoldAmount = -1;
    lastHpCost = -1;
    lastDmgCost = -1;
...
export function updateUI() {
  ...
  if (shouldUpdateShop()) {
    import('./shop/index.js').then(module => {
      if (module.updateShopUIForExternal) module.updateShopUIForExternal();
    });
  }
}
...
function updateBasicStats() {
  ...
  if (monstersVal) {
    ...
    } else value = state.monsters.length;
    
    if (value !== cache.monsters) {
      monstersVal.innerText = formatNumber(value);
  ...
  if (lvlVal) {
    ...
    if (value !== cache.level) {
      lvlVal.innerText = formatNumber(value);
      cache.level = value;
    }
  }
  ...
  if (hpVal && maxHpVal && hpBarFill) {
    ...
    if (Math.abs(hpPercent - cache.hpPercent) > 0.5) {
      ...
      if (hpPercent < 25) hpBarFill.style.background = 'linear-gradient(90deg, #c0392b, #e74c3c)';
      else if (hpPercent < 50) hpBarFill.style.background = 'linear-gradient(90deg, #e67e22, #f1c40f)';
      else hpBarFill.style.background = 'linear-gradient(90deg, #27ae60, #2ecc71)';
  ...
  if (staminaVal && staminaBarFill && staminaContainer) {
    ...
    if (staminaVal.innerHTML !== newText) staminaVal.innerHTML = newText;
    // Обновление полоски
    if (staminaBarFill.style.width !== staminaPercent + '%') staminaBarFill.style.width = staminaPercent + '%';
    ...
    // Проверка, нужно ли обновлять классы
    ...
    // Обновляется только если состояние изменилось
    if (isFull && !hasFull) staminaContainer.classList.add('full');
    else if (!isFull && hasFull) staminaContainer.classList.remove('full');

    if (isLow && !hasLow) staminaContainer.classList.add('low');
    else if (!isLow && hasLow) staminaContainer.classList.remove('low');

    if (isCritical && !hasCritical) staminaContainer.classList.add('critical');
    else if (!isCritical && hasCritical) staminaContainer.classList.remove('critical');
  }
  ...
  // Проверка, находится ли игрок в зоне без снегопада
  ...
  if (shouldShowWeatherIndicators && !isProtected) {
    ...
    if (snowIndicator) snowIndicator.style.display = 'inline-flex';
    // Убираем огненный индикатор, если он есть
    if (frostIndicator) frostIndicator.style.display = 'none';
    ...
    if (frostIndicator) frostIndicator.style.display = 'inline-flex';
    // Убираем обычный снежный индикатор
    if (snowIndicator) snowIndicator.style.display = 'none';
  } else {
  ...
  if (frostProgress <= 0) {
    ...
    if (iceOverlay && iceOverlay.parentNode) iceOverlay.parentNode.removeChild(iceOverlay);
  } else {
  ...
  if (goldVal) {
    ...
    if (value !== cache.gold) {
      goldVal.innerText = formatNumber(value);
      cache.gold = value;
    }
  }
  ...
  if (dmgVal) {
    ...
    if (value !== cache.damage) {
      dmgVal.innerText = formatNumber(value);
      cache.damage = value;
    }
  }
  ...
  if (state.isShopOpen) {
    ...
    if (hpCost) hpCost.innerText = formatNumber(player.hpCost) || 0;

    const dmgCost = document.getElementById('dmg-cost');
    if (dmgCost) dmgCost.innerText = formatNumber(player.dmgCost) || 0;
  }
}