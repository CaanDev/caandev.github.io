...
import { updateDescription, clearDescription, setLastSelectedItem } from './descriptionRenderer.js';
import { getWeaponIconHTML, getWeaponSlotIconHTML } from './utils.js';
    ...
    if (isOwned) {
      el.addEventListener('click', () => {
        ...
        if (weaponId) {
          ...
          if (player.meleeWeapon === weaponId && weaponId !== 'default') player.meleeWeapon = 'default';
          else if (weaponId === 'default' && player.meleeWeapon !== 'default') player.meleeWeapon = 'default';
          else if (weaponId !== 'default' && player.meleeWeapon !== weaponId) player.meleeWeapon = weaponId;
          ...
          if (window._inventoryRefreshCallback) window._inventoryRefreshCallback();
        }
      });
    }
  });
    ...
    if (isOwned) {
      el.addEventListener('click', () => {
        ...
        if (weaponId) {
          ...
          if (player.rangedWeapon === weaponId) player.rangedWeapon = null;
          else player.rangedWeapon = weaponId;
...
export function renderWeaponSlots() {
  ...
  if (primarySlot) {
    ...
    if (icon) {
      ...
      const iconHTML = getWeaponSlotIconHTML(weaponId, 100);
      ...
      // Добавление обработчика для показа описания
      ...
      // Перезаписывание переменной для дальнейшего использования
  ...
  if (secondarySlot) {
    ...
    if (icon) {
      ...
      if (weaponId) {
        const iconHTML = getWeaponSlotIconHTML(weaponId, 80);
        icon.innerHTML = iconHTML;
        icon.style.fontSize = '';
        icon.style.color = '';
        icon.style.opacity = '';
      ...
      // Добавление обработчика наведения для описания
