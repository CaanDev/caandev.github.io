...
export function updateDescription(weaponId) {
  ...
  if (isEquipped) title += ' <span class="equipped-status">(Экипировано)</span>';
  
  let subText = '';
  
  if (config.effects && config.effects.length > 0) subText += `Эффекты: ${config.effects.join(', ')}`;
...
export function updateDescriptionForItem(itemId) {
  // Проверка сначала снаряжения, потом предметов
  ...
  // Проверка, экипирован ли предмет (для снаряжения)
...
export function clearDescription() {
  const descContainer = document.getElementById('inventory-description');
  if (descContainer) descContainer.classList.remove('visible');
