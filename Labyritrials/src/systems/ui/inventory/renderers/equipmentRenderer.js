...
export function renderEquipmentGrid() {
  ...
  // Получение всего снаряжения (и экипированное, и нет)
  ...
  for (const itemId of equipmentItems) {
    ...
    // Проверка, экипирован ли предмет (в любом слоте)
  ...
  grid.querySelectorAll('.inventory-item-cell').forEach(el => {
    ...
    if (config) {
      ...
      el.addEventListener('click', () => {
        ...
        if (!player.inventory.equipped) player.inventory.equipped = { item1: null, item2: null, item3: null };
        ...
        if (isEquipped) {
          ...
          if (slot) delete player.inventory.equipped[slot];
        } else {
          ...
          if (freeSlot) {
            // Если есть свободный слот - экипируется в него
...
export function renderEquipmentSlots() {
  ...
  for (const slotId of equipmentSlots) {
    ...
    if (slot) {
      ...
      if (icon) {
        // Проверка, есть ли предмет в этом слоте
        ...
        // Добавление обработчика клика - снятие предмета
