...
export function renderItemGrid() {
  ...
  // Синхронизация карты перед рендерингом
  syncMap();
  
  const availableItems = player.inventory?.items?.available || [];
  
  // Если нет предметов - показывается сообщение
  ...
  // Проход по всем доступным предметам
  ...
  // Добавление обработчиков для каждой ячейки
