...
import { getEventFogColors } from '../../data/events.js';

/**
 * Получение цветов эффектов для события
 */
function getEventColors(eventId) {
  const colors = getEventFogColors(eventId);
  if (!colors) {
    // Fallback на случай, если цвета не найдены
    return {
      color1: 'rgba(255, 255, 255, ',
      color2: 'rgba(200, 200, 200, ',
      color3: 'rgba(150, 150, 150, ',
      color4: 'rgba(100, 100, 100, 0)',
      spark: '#ffffff',
      sparkShadow: '#ffffff',
    };
  }
...
export function drawEventFogEffect(ctx, canvas) {
  ...
  switch (event) {
    case 'bloodMoon':
      drawBloodMoonEffect(ctx, canvas, time, colors);
      break;
    case 'iceWind':
      drawIceWindEffect(ctx, canvas, time, colors);
      break;
    case 'blessing':
      drawBlessingEffect(ctx, canvas, time, colors);
      break;
    case 'monsterRage':
      drawMonsterRageEffect(ctx, canvas, time, colors);
      break;
    case 'fragility':
      drawFragilityEffect(ctx, canvas, time, colors);
      break;
  }
}
 ...
 * @param {Object} colors - Цвета эффекта
 * @returns {void}
 * @private
 */
function drawBloodMoonEffect(ctx, canvas, time, colors) {
 ...
 * @private
 */
function drawIceWindEffect(ctx, canvas, time, colors) {
  ...
  frostGradient.addColorStop(0, 'rgba(0, 0, 0, 0)');
  frostGradient.addColorStop(0.7, 'rgba(0, 0, 0, 0)');
 ...
 * @private
 */
function drawBlessingEffect(ctx, canvas, time, colors) {
 ...
 * @private
 */
function drawMonsterRageEffect(ctx, canvas, time, colors) {
 ...
 * @private
 */
function drawFragilityEffect(ctx, canvas, time, colors) {
