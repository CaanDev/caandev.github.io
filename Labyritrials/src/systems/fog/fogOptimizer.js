 ...
 */
const ZONES = {
  CLOSE: { maxDist: 3, interval: 1 },      // Близко — обновляется каждый кадр
...
export function updateFogMemorySmart() {
  ...
  const revealRadius = currentRadius + CONFIG.cellSize * 5;
  
  const playerMoved = (playerGridX !== cache.playerX || playerGridY !== cache.playerY);
  
  // Если игрок не двигался и прошло мало времени - используется кэш
  if (!playerMoved && (frameCounter - cache.lastUpdateFrame) < 30) {
    updateCachedCells(frameCounter, revealRadius);
    return;
  }
  ...
  for (let y = startY; y < endY; y++) {
    for (let x = startX; x < endX; x++) {
      ...
      cellsToUpdate.push({
        ...
        isRevealable: dist < revealRadius,
  ...
  updateCells(cellsToUpdate, frameCounter, revealRadius);
}
 ...
 * @private
 */
function updateCachedCells(frameCount, revealRadius) {
  ...
  for (const item of cache.cellsToUpdate) {
    ...
    if ((frameCount % item.interval) === hash) updateSingleCell(item, revealRadius);
  }
}
 ...
 * @private
 */
function updateCells(cells, frameCount, revealRadius) {
 ...
 * @private
 */
function updateSingleCell(item, revealRadius) {
  ...
  // Открытие клетки, если она в радиусе открытия (revealRadius)
  // и она ещё не открыта (cell.revealed === false)
  if (!cell.revealed && dist < revealRadius) {
    cell.revealed = true;
    cell.lastSeenFrame = frameCounter;
    cell.fogIntensity = 0;
  }
  
  // Обновление затухания для уже открытых клеток
  if (cell.revealed) {
    if (isVisible) {
      ...
      // Клетка вне радиуса - рассчёт затухания
