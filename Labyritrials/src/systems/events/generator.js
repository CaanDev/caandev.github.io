...
export function generateRandomEvent() {
  ...
  // Если нет доступных событий - выход
...
export function clearEventEffects() {
  // Если события нет - ничего не делается
  if (!state.currentEvent) return;
  
  // Снятие эффекта события
  const event = EVENTS_DATA[state.currentEvent];
  if (event) event.effects.remove(player, state);
