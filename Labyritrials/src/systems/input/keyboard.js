...
export function handleKeyDown(e) {
  // Если открыто любое модальное окно - блокировка всех клавиш
  if (isAnyModalOpen()) {
    e.preventDefault();
    e.stopPropagation();
    return;
  ...
  if (e.key === 'Escape') {
    ...
    const levelUpUI = document.getElementById('level-up-ui');
    const gameOverUI = document.getElementById('game-over-ui');
    if (levelUpUI?.style.display === 'block' || gameOverUI?.style.display === 'block') return;
...
async function handleShopToggle() {
  ...
  if (distToShop < CONFIG.cellSize) {
    // Если магазин уже открыт - ничего не делается
    if (state.isShopOpen) return;

    // Загрузка шаблона магазина (если нужно)
    if (!isTemplateLoaded('shop')) await loadTemplateIfNeeded('shop');

    // Импорт модуля магазина
    const shopModule = await import('../ui/shop/index.js');
    
    // Инициализация обработчиков
    if (shopModule.initShopHandlers) shopModule.initShopHandlers();
    ...
    // Регистрация открытия модального окна
    const { registerModalOpen } = await import('../ui/modalManager.js');
    registerModalOpen('shop');
    
    // Обновление UI магазина (включая спрайты)
    if (shopModule.updateShopUIForExternal) setTimeout(shopModule.updateShopUIForExternal, 50);
...
export async function openNoteWindow(noteId) {
  ...
  // Регистрирация открытия модального окна
  const { registerModalOpen } = await import('../ui/modalManager.js');
  registerModalOpen('note');

  if (!isTemplateLoaded('noteWindow')) await loadTemplateIfNeeded('noteWindow');
  ...
  if (state.notes.positions && state.notes.positions[noteId]) delete state.notes.positions[noteId];
...
export async function closeNoteWindow() {
  ...
  // Регистрация закрытия модального окна
