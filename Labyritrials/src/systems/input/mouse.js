...
function isClickOnControlButton(target) {
  ...
  while (element && element !== document.body) {
    if (controlButtons.includes(element.id)) return true;
...
export function updateMousePosition(e) {
  ...
  // Обновление направления атаки игрока
  const dx = mouseWorldX - player.px;
  const dy = mouseWorldY - player.py;
  const length = Math.hypot(dx, dy);
  
  // Всегда обновляется направление, даже если длина < 10
  if (length > 5) {
    ...
    // Если курсор слишком близко к игроку - используется последнее направление
  ...
  // Всегда обновляется последнее направление при движении мыши
...
export function handleMouseDown(e) {
  // Если открыто модальное окно - пропуск кликов вне окна
  if (isAnyModalOpen()) {
  ...
  if (e.button === 0 && !state.isShopOpen) {
    ...
    // Обновление направления при клике
    const canvas = document.getElementById('gameCanvas');
    const rect = canvas.getBoundingClientRect();
    ...
    if (length > 10) {
      player.dirX = dx / length;
      player.dirY = dy / length;
      // Всегда обновляется lastMoveDir при клике
      player.lastMoveDirX = player.dirX;
      player.lastMoveDirY = player.dirY;
    } else {
      // Если клик прямо на игрока - используется последнее направление
      if (player.lastMoveDirX !== 0 || player.lastMoveDirY !== 0) {
        // lastMoveDir уже есть, ничего не делается
      } else {
...
export function handleMouseUp(e) {
  ...
  if (e.button === 0 && player.isCharging) {
    ...
    if (player.stamina < staminaCost) {
      // Недостаточно выносливости - анимация атаки не запускается
