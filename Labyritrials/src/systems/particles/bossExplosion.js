...
export function createBossExplosion(x, y, isBoss = false, bossType = 'demon') {
  if (!state.bossExplosions) state.bossExplosions = [];
  ...
  if (isMindBoss) colors = ['#44aaff', '#66ccff', '#88ddff', '#9b59b6', '#bb8af0', '#dda0dd'];
  else colors = ['#ff2200', '#ff4400', '#ff6600', '#ff8800', '#ffaa00', '#ffcc00'];
  ...
  for (let i = 0; i < particleCount; i++) {
    ...
    if (isMindBoss && i % 4 === 0) state.bossExplosions.push(new BossExplosionParticle(x, y, isBoss, 'magic_core', colors, true));
  }
...
export function updateBossExplosions() {
  ...
  for (let i = state.bossExplosions.length - 1; i >= 0; i--) {
    ...
    if (!alive) state.bossExplosions.splice(i, 1);
  }
  ...
  if (state.shockwave) {
    ...
    if (state.shockwave.life <= 0 || state.shockwave.radius >= state.shockwave.maxRadius) state.shockwave = null;
  }
}
...
export function drawShockwave(ctx, camX, camY) {
  ...
  if (sw.life <= 0 || sw.radius >= sw.maxRadius) state.shockwave = null;
}
