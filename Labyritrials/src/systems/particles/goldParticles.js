...
export function createGoldParticles(x, y, value, biome = 'cave', simple = true) {
  ...
  let count;
  count = (simple) ? Math.min(20, 8 + Math.floor(value / 10)) : Math.min(35, 12 + Math.floor(value / 6));
...
/**
 * Обновление целевой позиции для частиц золота (при изменении размеров окна)
 * 
 * @returns {void}
 */
...
export function updateGoldParticles() {
  ...
  goldParticlePool.updateAll((p) => {
    ...
    if (p.attractionDelay > 0) p.attractionDelay--; else p.attracted = true;
    // Смерть частицы
