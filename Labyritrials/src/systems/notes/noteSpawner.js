...
export function spawnNotes() {
  ...
  if (state.notes.spawned[level] && state.notes.spawned[level].length > 0) {
    ...
    // Если записки есть на карте - ничего не делается
    if (notesOnMap > 0) return;

    // Если записок на карте нет - очистка spawned и повторный спавн
    state.notes.spawned[level] = [];
  }

  // Инициализация структур данных, если их нет
  if (!state.notes.spawned[level]) state.notes.spawned[level] = [];
  if (!state.notes.positions) state.notes.positions = {};

  let spawnedCount = 0;
  // Для отслеживания позиций уже заспавненных записок
  const existingPositions = [];

  for (const noteData of notesForLevel) {
    // Пропуск уже найденных записок
    if (state.notes.found.includes(noteData.id)) continue;
    // Пропуск уже заспавненных на этом уровне
    if (state.notes.spawned[level].includes(noteData.id)) continue;

    // Проверка, есть ли уже сохранённая позиция для этой записки
    if (state.notes.positions[noteData.id]) {
      const pos = state.notes.positions[noteData.id];
      const cell = state.grid[pos.y]?.[pos.x];
      
      // Проверка, что клетка подходит для размещения записки
      if (cell && cell.isWall && !cell.isBreakable && !cell.hasNote) {
        ...
        // Если позиция невалидна - удаление из сохранения
        delete state.notes.positions[noteData.id];
      }
    }

    // Поиск новой позиции для записки
...
function findNoteSpawnPosition(noteId = null, existingPositions = []) {
  ...
  // Минимальное расстояние между записками в клетках
  const minDistanceBetweenNotes = 5;
  ...
  for (let attempt = 0; attempt < attempts; attempt++) {
    ...
    if (noteId !== null) {
      // Создание уникального seed для каждой записки для детерминированного результата
      ...
      // Если noteId не передан - используется обычный рандом
