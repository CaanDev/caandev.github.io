...
export function saveGame() {
  // Запрет сохранения во время активной волны в комнате-ловушке
  ...
  // Если игра завершена или игрок мёртв — запрет сохранения
...
export async function loadGame() {
  ...
  try {
    ...
    // Восстановление погоды после всех данных
    ...
    // Восстановление всего остального
    ...
    // Восстановление снегопада
    if (weatherRestored && snowState.active) {
      // Проверка, что игрок в ледяном биоме и не в тайной комнате
      ...
      if (isIceBiome && !isInSecretRoom && !state.isBossLevel) {
        // Импорт функции для создания снежинок
        const { createSnowflakes } = await import('../systems/weather/snowRenderer.js');
        createSnowflakes();
        
        // Установка прозрачности на максимум
        snowState.targetOpacity = 1;
        snowState.opacity = 1;
        
        // Восстановление таймера остановки снегопада
        const elapsed = Date.now() - snowState.startTime;
        // Используются значения из snowState или значения по умолчанию
        ...
        // Если не подходит биом — отключение снегопада
        snowState.active = false;
        snowState.targetOpacity = 0;
        snowState.opacity = 0;
      }
    ...
    if (Game && Game.updateUI) Game.updateUI();
    ...
    if (state.inSafeRoom) audio.forcePlayMusic('safeRoom');
    else if (state.inTreasureRoom || state.inShrineRoom || state.inTrapRoom) audio.forcePlayMusic('game');
    else audio.forcePlayMusic('game');
...
async function applySettingsAfterLoad() {
  try {
    ...
    if (typeof updateFpsLimit === 'function') updateFpsLimit();
  } catch (e) {
...
async function restoreMimicFlies() {
  ...
  // Восстановление мух над мимиками
