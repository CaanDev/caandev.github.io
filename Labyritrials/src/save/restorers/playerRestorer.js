...
export function restorePlayerData(save) {
  ...
  if (save.gender) {
    ...
    import('../../sprites/index.js').then(({ playerAnimator }) => {
      playerAnimator.setGender(save.gender);
    });
  }
  ...
  // Восстановление гендерных характеристик
  if (save.evasionBonus !== undefined) player.evasionBonus = save.evasionBonus;
  if (save.genderBonuses) player.genderBonuses = save.genderBonuses;
  if (save.staminaRegenInCombat !== undefined) player.staminaRegenInCombat = save.staminaRegenInCombat;
  if (save.staminaRegenOutOfCombat !== undefined) player.staminaRegenOutOfCombat = save.staminaRegenOutOfCombat;
}
