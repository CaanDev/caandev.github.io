...
export function restoreTreasureRoomData(save) {
  ...
  if (tr.originalGrid) restoreOriginalMazeGrid(tr.originalGrid, tr.originalMapCols, tr.originalMapRows);
...
export function restoreSecretRoomsData(save) {
  ...
  // Восстановление названия комнаты
...
export function restoreTrapRoomData(save) {
  ...
  if (state.inTrapRoom && state.trapActivated) {
    ...
    // Добавление восстановленных монстров
    for (const m of state.trapMonsters) {
      if (m.hp === undefined || m.hp <= 0) m.hp = m.maxHp || 50;
      if (m.maxHp === undefined || m.maxHp <= 0) m.maxHp = m.hp || 50;

      m.isTrapMonster = true;
      m.canDropItems = false;

      if (!m.id) m.id = `${m.x},${m.y},${Date.now()}`;
      state.trapMonsterIds.add(m.id);

      // Проверка, нет ли уже такого монстра
      const exists = state.monsters.some(existing => 
        existing.x === m.x && existing.y === m.y && existing.id === m.id
      );
      if (!exists) state.monsters.push(m);
    }

    // Восстановление состояния волны
    ...
    // Если игрок не в комнате-ловушке
    state.trapMonsters = [];
    state.trapWaveActive = false;
    state.trapActivated = false;
  }
...
export function restoreSafeRoomData(save) {
  ...
  if (save.originalHadMap !== undefined) state.originalHadMap = save.originalHadMap;
  
  if (save.originalRevealedCells && Array.isArray(save.originalRevealedCells)) state.originalRevealedCells = save.originalRevealedCells;
  else state.originalRevealedCells = [];
  ...
  if (save.safeChestOpened !== undefined) state.safeChestOpened = save.safeChestOpened;
