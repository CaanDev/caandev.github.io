...
export function restoreMonstersData(save) {
  // Если игрок в комнате-ловушке и волна активна — монстры не перезаписываются
  if (save.trapRoomData?.inTrapRoom && save.trapRoomData?.trapWaveActive) {
    // Добавление недостающих монстров из сохранения
  ...
  if (save.monsters && Array.isArray(save.monsters)) {
    state.monsters = save.monsters.map(m => {
      ...
      if (restored.isBoss || restored.isDuoBoss) restoreBossAbilities(restored);
...
export function restoreBossData(save) {
  ...
  if (save.bossSpawned !== undefined) state.bossSpawned = save.bossSpawned;
  if (save.bossSpawnTriggered !== undefined) state.bossSpawnTriggered = save.bossSpawnTriggered;
  if (save.bossSpawnTimer !== undefined) state.bossSpawnTimer = save.bossSpawnTimer;
}