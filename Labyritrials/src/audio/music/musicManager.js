...
class MusicManager {
          ...
          .catch((err) => {
            ...
            if (err.name === 'NotAllowedError') {
              ...
              if (this.playAttempts < this.maxPlayAttempts) {
                setTimeout(() => {
                  if (!this.isPlaying && !this._isLoading) this.play(mode);
                }, 1000);
              }
              return;
            }
  ...
  pause() {
    ...
    try {
      ...
      if (err.name !== 'AbortError') logger.warn('⚠️ Не удалось поставить на паузу:', err);
    }
  }
  ...
  stop() {
    ...
    if (this.currentTrack) {
      try {
        ...
        if (err.name !== 'AbortError') logger.warn('⚠️ Не удалось остановить музыку:', err);
      }
    }
  ...
  setVolume(value) {
    ...
    // Обновление громкости у всех загруженных треков
    ...
    // Обновление текущего трека, если он есть
    if (this.currentTrack) {
  ...
  reload() {
    ...
    // Перезагрузка трека
    const config = getTrackConfig(this.currentMode);
    if (config) musicLoader.loadTrack(this.currentMode);

    setTimeout(() => {
      if (this.isEnabled) this.play(this.currentMode);
    }, 100);
  }
  ...
  updateState() {
    ...
    if (this.isEnabled && !this.isPlaying && this.currentTrack && !this._isLoading) this.resume();
    else if (!this.isEnabled && this.isPlaying) this.pause();
  }
