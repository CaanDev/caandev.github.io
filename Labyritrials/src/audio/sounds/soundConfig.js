...
// Сбор всех звуков в плоскую структуру
...
export function getSoundConfig(key) {
  ...
  if (FLAT_SOUNDS[key]) return FLAT_SOUNDS[key];

  // Попытка найти в иерархии
  const parts = key.split('.');
  let current = SOUNDS;

  for (const part of parts) {
    if (current && typeof current === 'object' && current[part]) current = current[part]; else return null;
  }

  // Проверка того, что звук найден (имеет path)
