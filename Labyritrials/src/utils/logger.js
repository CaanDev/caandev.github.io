...
/**
 * Режим отладки.
 * Установить в false для отключения всех логов (кроме ошибок).
 * 
 * @type {boolean}
 */
const DEBUG = true;
...
function logWithColor(label, color, message, ...args) {
  if (args.length > 0) console.log(`%c${label}%c ${message}`, `color: ${color}; font-weight: bold;`, 'color: inherit;', ...args);
  else console.log(`%c${label}%c ${message}`, `color: ${color}; font-weight: bold;`, 'color: inherit;');
}
...
function warnWithColor(label, color, message, ...args) {
  if (args.length > 0) console.warn(`%c${label}%c ${message}`, `color: ${color}; font-weight: bold;`, 'color: inherit;', ...args);
  else console.warn(`%c${label}%c ${message}`, `color: ${color}; font-weight: bold;`, 'color: inherit;');
}
...
function errorWithColor(label, color, message, ...args) {
  if (args.length > 0) console.error(`%c${label}%c ${message}`, `color: ${color}; font-weight: bold;`, 'color: inherit;', ...args);
  else console.error(`%c${label}%c ${message}`, `color: ${color}; font-weight: bold;`, 'color: inherit;');
}
...
export const logger = {
   ...
   */
  info(message, ...args) {
    if (isLevelActive(LOG_LEVELS.INFO)) logWithColor('ℹ️ INFO', '#4CAF50', message, ...args);
  },
  ...
  success(message, ...args) {
    if (isLevelActive(LOG_LEVELS.INFO)) logWithColor('✅ SUCCESS', '#4CAF50', message, ...args);
  },
   ...
   */
  warn(message, ...args) {
    if (isLevelActive(LOG_LEVELS.WARN)) warnWithColor('⚠️ WARN', '#FFC107', message, ...args);
  },
   ...
   */
  debug(message, ...args) {
    if (isLevelActive(LOG_LEVELS.DEBUG)) logWithColor('🔍 DEBUG', '#2196F3', message, ...args);
  },
   ...
   */
  game(message, ...args) {
    if (isLevelActive(LOG_LEVELS.INFO)) logWithColor('🎮 GAME', '#9C27B0', message, ...args);
  },
   ...
   */
  save(message, ...args) {
    if (isLevelActive(LOG_LEVELS.INFO)) logWithColor('💾 SAVE', '#00BCD4', message, ...args);
  },
  ...
  achievement(message, ...args) {
    if (isLevelActive(LOG_LEVELS.INFO)) logWithColor('🏆 ACHIEVEMENT', '#FFD700', message, ...args);
  },
