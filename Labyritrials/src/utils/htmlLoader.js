...
const TEMPLATE_MAP = {
  ...
  // windows/
  'characterSelect': 'windows/characterSelect',
...
export async function loadTemplate(name) {
  try {
    // Использование маппинга для определения пути
    const mappedName = TEMPLATE_MAP[name] || name;
    const response = await fetch(`${HTML_PATH}${mappedName}.html`);
    
    if (!response.ok) throw new Error(`Не удалось загрузить шаблон: ${name} (${response.status})`);
...
export async function initTemplateHandlers(name) {
  // Если уже инициализирован - пропуск
  ...
  switch (name) {
    case 'characterSelect':
      // Не требуется дополнительной инициализации
      success = true;
      break;
    ...
    case 'inventory':
      // Без дополнительной логики, только базовое открытие/закрытие
      success = true;
      break;
    ...
    case 'notification':
    case 'confirm':
      // Не требуются инициализации обработчиков
      success = true;
      break;
  ...
  if (success) initializedTemplates.set(name, true);
...
export async function loadEssentialTemplates() {
  ...
  // Инициализация обработчиков для критических шаблонов
  for (const name of ESSENTIAL_TEMPLATES) {
    // Извлечение имени без пути для инициализации
...
export async function loadTemplateIfNeeded(name) {
  // Уже загружен - возврат из кэша
  if (loadedTemplates.has(name)) return templateCache.get(name) || null;
  
  // Уже загружается - ожидание
  ...
  if (html && html.trim().length > 0) {
    ...
    // Важно: Инициализация обработчиков после вставки в DOM
    // Извлечение имени без пути для инициализации
    const shortName = name.includes('/') ? name.split('/').pop() : name;
    await initTemplateHandlers(shortName);
