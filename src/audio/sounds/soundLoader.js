...
class SoundLoader {
  ...
  async loadSound(key) {
    ...
    try {
      ...
      // Ожидание загрузки (или ошибки)
      await new Promise((resolve, reject) => {
        audio.addEventListener('canplaythrough', resolve, { once: true });
        audio.addEventListener('error', (e) => {
          // Некоторые браузеры могут не поддерживать canplaythrough для OGG
          if (audio.readyState >= 2) resolve(); else reject(e);
        }, { once: true });
        audio.load();
      });

      // Проверка размера кэша
      if (this.cache.size >= this.maxCacheSize) this._evictOldest();
      ...
      this.loading.delete(key);
      // Не выводится ошибка для каждого звука, только если их мало
      if (this.loading.size < 5) logger.warn(`⚠️ Не удалось загрузить звук "${key}":`, error);
      return null;
    }
  }
