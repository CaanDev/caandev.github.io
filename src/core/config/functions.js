...
export function resetGameFull() {
  ...
  player.staminaRegenInCombat = 6;
  player.staminaRegenOutOfCombat = 10;
  player.staminaUpgradeCost = 150;
  ...
  // Сброс гендерных характеристик
  if (player.gender) {
    import('../../data/genderConfig.js').then(({ getGenderConfig }) => {
      const config = getGenderConfig(player.gender);
    ...
    // Fallback для мужского (по умолчанию)
    player.dmgCost = 40;
  ...
  if (state.keys) state.keys['mouse0'] = false;
