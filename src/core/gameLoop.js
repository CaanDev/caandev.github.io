...
export function createGameLoop(updateUICallback, ctx, canvas) {
  return function gameUpdate(timestamp) {
    ...
    // Ограничение deltaTime, чтобы избежать скачков
    const clampedDeltaTime = Math.min(deltaTime, 1/30);
    
    // Если открыто окно перехода уровня — только отображение
    ...
    // Если идёт респавн — его обработка
    if (state.isRespawning) {
      handleRespawn();
      return;
    }
    
    // Если игрок мёртв — просто отрисовка и выход
    if (player.hp <= 0) {
      Renderer.draw(ctx, canvas);
      return;
    }
    ...
    // Обновление снегопада (проверка на старт/остановку)
    if (frameCounter % 30 === 0) updateSnowfall();
    // Обновление прозрачности снега
    updateSnowOpacity();
...
function handleRespawn() {
  ...
  setTimeout(() => {
    showEventMessage();
    if (Game && Game.startLoop) Game.startLoop();
  }, 100);
}
...
function updateSparks() {
  ...
  for (let i = state.sparks.length - 1; i >= 0; i--) {
    ...
    if (s.gravity !== undefined && s.gravity !== 0) s.vy += s.gravity;
    else if (s.gravity === undefined) s.vy += 0.15;
