...
export const Game = {
  ...
  startLoop() {
    ...
    if (this.vsyncEnabled) this._runVsyncLoop(); else this._runUnlimitedLoop();
  },
  ...
  _runVsyncLoop() {
    ...
    function animate(timestamp) {
      ...
      // Проверка, не переключился ли режим
      const settings = getSettings();
      if (!settings.vsyncEnabled) {
        // Переключение на режим без VSync
  ...
  _runUnlimitedLoop() {
    ...
    function tick() {
      ...
      // Планирование следующего кадра как можно скорее
