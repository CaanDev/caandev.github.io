...
export function spawnMonsters(isTreasureRoom = false, isProtectedCell = () => false) {
  ...
  for (const cell of cells) {
    ...
    if (state.bloodMoonActive) newMonster.hasVampirism = true;
...
export function spawnArtifacts(isTreasureRoom = false, isProtectedCell = () => false) {
  ...
  let artifactBiome;
  if (isTreasureRoom || state.inTreasureRoom) artifactBiome = 'treasure';
  else artifactBiome = state.currentBiome || 'cave';
  ...
  for (let i = 0; i < Math.min(artifactCount, cells.length); i++) {
    const cell = cells[i];
    const spriteName = getRandomSprite('artifact', artifactBiome) || 'artifactCave1';
