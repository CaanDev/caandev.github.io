...
import { spawnDamageBloodTrail } from './bloodTrails.js';
...
function isBlockedByMonster(target, blocker) {
  ...
  // Используется средний радиус обоих монстров
  const avgRadius = (blockerRadius + targetRadius) / 2;
  const threshold = avgRadius + 10;
  
  // Если блокер почти на линии, считается что перекрывает
...
export function executeAttack(isStrong) {
  // Если игрок отключен - нет атаки
  if (state.isPlayerDisabled) return;

  // Блокировка атаки, если босс появляется
...
function hasPillarBetween(x1, y1, x2, y2) {
  ...
  for (let i = 1; i < steps; i++) {
    ...
    if (cy >= 0 && cy < CONFIG.rows && cx >= 0 && cx < CONFIG.cols) {
      if (state.grid[cy] && state.grid[cy][cx] && state.grid[cy][cx].isPillar) return true;
    }
  }
  return false;
}
...
function dealDamageToMonsters(attackX, attackY, damage, isStrong, dirX, dirY) {
  // Обновление счётчика попаданий для оружия
  const weaponType = player.meleeWeapon || 'default';
  if (weaponType === 'default') state.gameStats.weaponHits.default++;
  else if (weaponType === 'stun') state.gameStats.weaponHits.stun++;
  else if (weaponType === 'vampire') state.gameStats.weaponHits.vampire++;

  // Определение максимального количества целей
  const maxTargets = WEAPON_MAX_TARGETS[weaponType] || 2;

  // Ближнее оружие: сбор монстров в радиусе
  ...
  // Сортировка по расстоянию от игрока
  monstersInRange.sort((a, b) => a.distToPlayer - b.distToPlayer);

  // Нанесение урона только передней линии
  ...
  for (const item of monstersInRange) {
    ...
    // Если достигнут лимит целей — остановка
    if (targetsHit >= maxTargets) break;
    
    // Проверка, не перекрыт ли этот монстр другим (уже обработанным) монстром
    let isBlocked = false;
...
function applyDamageToMonster(m, index, damage, isStrong) {
  // Запоминание времени атаки (для определения статуса "в бою")
...
function applyWeaponEffects(m, damage, isStrong) {
  ...
  if (player.meleeWeapon === 'stun') {
    if (!hasStunImmunity() && !m.isBoss && !m.isDuoBoss) {
      ...
      // Бонус (мужской персонаж): оглушение на 0.5 сек дольше
      if (player.genderBonuses?.stunDurationBonus) stunDuration += Math.floor(player.genderBonuses.stunDurationBonus * 60);
  ...
  if (player.meleeWeapon === 'vampire') {
    ...
    if (isStrong) healPercent = player.vampireStrongPercent + (player.artifactsCollected * player.vampireArtifactBonus);
    else healPercent = player.vampireBasePercent + (player.artifactsCollected * player.vampireArtifactBonus);
    
    // Бонус (женский персонаж): лечение +2%
    if (player.genderBonuses?.vampHealBonus) healPercent += player.genderBonuses.vampHealBonus * 100;
...
export function playerTakeDamageFromMonster(damage) {
  ...
  if (player.incomingDamageMultiplier) actualDamage = Math.floor(actualDamage * player.incomingDamageMultiplier);
  player.hp -= actualDamage;

  // Создание следов крови при получении урона
  spawnDamageBloodTrail(player.px, player.py, actualDamage);
