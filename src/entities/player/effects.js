...
function updateLowHPEffect() {
  ...
  if (typeof player.hp !== 'number' || typeof player.maxHp !== 'number' || player.maxHp <= 0) return;
  
  const hpPercent = Math.max(0, Math.min(1, player.hp / player.maxHp));
  
  // Обновление звука сердцебиения
...
export function updateFreezeEffect() {
  if (player.freezeTimer > 0) {
    ...
    // Если игрок только что замёрз — заморозка анимации
...
export function updatePlayerEffects() {
  ...
  if (player.trapGlowTimer > 0) {
    ...
    if (player.trapGlowTimer <= 0) player.trapGlowColor = null;
  }
  ...
  // Проверка смерти (остановка звука шока)
  if (player.hp <= 0) {
  ...
  // Обновление эффекта низкого здоровья
  updateLowHPEffect();
}
...
export function updateShockEffect() {
  // Проверка смерти в начале каждого кадра
  if (player.hp <= 0) {
  ...
  if (player.shockTimer <= 0) {
    ...
    if (state.shockSparks && state.shockSparks.length > 0) state.shockSparks = [];
    return false;
  }
  ...
  // Замедление
  if (player.originalSpeed === undefined) player.originalSpeed = player.baseSpeed;
...
export function updatePoisonEffect() {
  if (player.poisonTimer <= 0) {
    // Очищение пузырьков, если отравление закончилось
    if (state.poisonBubbles && state.poisonBubbles.length > 0) state.poisonBubbles = [];
    return false;
  }
  ...
  if (player.poisonTimer > 0) player._wasPoisoned = true;
  return false;
}