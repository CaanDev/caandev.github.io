...
export function collectLoot() {
  for (let i = state.lootItems.length - 1; i >= 0; i--) {
    ...
    // Пропуск интерактивных предметов (их подбирают по E)
    if (item.requiresInteraction === true) continue;

    if (Math.hypot(player.px - item.x, player.py - item.y) < 35) {
      if (item.type === 'gold') handleGoldPickup(item);
      else if (item.type === 'potion') handlePotionPickup(item);
...
function handleGoldPickup(item) {
  ...
  // Определение биома для частиц золота
...
function handlePotionPickup(item) {
  ...
  // Обновление прогресса достижений (только если зелье реально вылечило)
  if (actualHeal > 0) updateProgress('potions_collected', 1);
