...
function getMimicsInRange(attackX, attackY, radius) {
  ...
  for (const mimic of state.mimics) {
    ...
    if (dist < radius) mimicsInRange.push(mimic);
  }
...
function dealDamageToMimic(mimic, damagePercent) {
  ...
  // Открытие мимика при получении урона
  mimic.opened = true;
  // Сброс таймера закрытия (будет закрыт через 500мс)
  mimic.closeTimer = Date.now() + 500;
  
  const damage = Math.floor(mimic.maxHp * damagePercent);
  mimic.hp -= damage;
  
  // Отображение полоски HP после первого попадания
...
export function updateMimicsState() {
  ...
  for (const mimic of state.mimics) {
    ...
    // Закрытие мимика через 500мс после открытия
...
export function dealDamageToMimicsMelee(attackX, attackY, attackRadius) {
  ...
  // Ближний бой — наносится урон только одному мимику (ближайшему)
  ...
  if (died) handleMimicDeath(closestMimic);
...
export function updateMimicHealthBars() {
  ...
  for (const mimic of state.mimics) {
    ...
    // Если HP полное или больше максимального — скрытие полоски
    if (mimic.hp >= mimic.maxHp) {
      mimic.hpBarVisible = false;
      continue;
    }
    
    // Если полоска уже видима - проверка таймера
    if (mimic.hpBarVisible) {
      // Если прошло больше HIDE_DELAY с последнего попадания - скрытие
