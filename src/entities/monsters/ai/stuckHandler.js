...
export function checkAndHandleStuck(m, distToMemory) {
  ...
  // Проверка, сдвинулся ли монстр
  if (m.x === m._lastX && m.y === m._lastY) m._stuckCounter++; else m._stuckCounter = 0;
  
  // Если застрял более чем на 30 кадров
  if (m._stuckCounter > 30) {
    // Отображение того, что монстр обдумывает путь
    ...
    // Если слишком долго стоит (> 60 кадров) — переход в патруль
    if (m._stuckCounter > 60) return transitionToPatrol(m, 'долгого раздумья');
    
    // Не двигается — просто стоит
    m._lastX = m.x;
    m._lastY = m.y;
...
export function transitionToPatrol(m, reason = '') {
  // Если монстр реагирует на звук - ? не показывается
  if (m._isReactingToSound) {
    // Простой сброс состояния без визуального эффекта
    m.isSearching = false;
    m.lastKnownX = null;
    m.lastKnownY = null;
    m.searchTimer = 0;
