...
export function updatePredictedPath(m, playerGridX, playerGridY) {
  ...
  if (Math.abs(dx) > Math.abs(dy)) dirX = Math.sign(dx); else dirY = Math.sign(dy);
...
export function advancePredictionStep(m, currentX, currentY) {
  ...
  if (currentStep >= m.predictedPath.length - 1) return true;
  ...
  if (dist < 35) m.predictionStep = currentStep + 1;
  return false;
}