...
import { playerTakeDamageFromMonster } from '../player/combat.js';
...
export function handleMonsterDamageToPlayer(m, now) {
  // Если игрок отключен - монстры не атакуют
  if (state.isPlayerDisabled) return false;

  const distToPlayer = Math.hypot(player.px - m.x, player.py - m.y);

  // Проверка, находится ли игрок в радиусе атаки и прошёл ли кулдаун
  if (distToPlayer < m.radius + 24 && (now - m.lastHit > 1000)) {
    ...
    // Сброс флага "Железный человек" (достижение)
