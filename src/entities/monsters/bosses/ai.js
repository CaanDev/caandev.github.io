  ...
  for (let dy = -checkRadius; dy <= checkRadius; dy++) {
    for (let dx = -checkRadius; dx <= checkRadius; dx++) {
      ...
      if (isPillarAt(gridX + dx, gridY + dy)) {
        ...
        if (dist < radius + CONFIG.cellSize * 0.4) return true;
      }
    }
  }
  return false;
...
export function updateBossState(m, distToPlayer) {
  ...
  switch (m.bossType) {
    case BOSS_TYPES.DEMON: {
      // Демон: поведение зависит от фазы
      // Фаза 1 — преследование, фазы 2-3 - избегание
      const isPhase2Or3 = m.currentPhase === 2 || m.currentPhase === 3;
...
function getWallAvoidanceDirection(m, gridX, gridY, isNearTopWall, isNearBottomWall, isNearLeftWall, isNearRightWall) {
  ...
  if (isNearTopWall) targetY = m.y + CONFIG.cellSize * 2;
  else if (isNearBottomWall) targetY = m.y - CONFIG.cellSize * 2;

  if (isNearLeftWall) targetX = m.x + CONFIG.cellSize * 2;
  else if (isNearRightWall) targetX = m.x - CONFIG.cellSize * 2;
...
export function updateBossMovement(m) {
  ...
  if (m.isTremoring) {
    if (m.tremorDuration > 0) {
      ...
      if (m.tremorDuration <= 0) m.isTremoring = false;
    }
  }
