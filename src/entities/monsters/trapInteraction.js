...
function performMonsterDodge(m, t) {
  // Выбор направления уворота (от ловушки)
  const angle = Math.atan2(m.y - t.y, m.x - t.x);
  const dodgeDistance = 50 + Math.random() * 30;
  
  // Проба разных направлений (отклонения от основного)
  const directions = [
  ...
  // Перемешивание направлений для случайности
  ...
  for (const dir of directions) {
    // Проверка нескольких точек по пути уворота
    const steps = 5;
    let canDodge = true;
    let finalX = m.x;
    let finalY = m.y;
    ...
    for (let step = 1; step <= steps; step++) {
      ...
      // Проверка коллизии со стенами и колоннами
      ...
      // Проверка, не выходит ли монстр за границы карты
  ...
  // Если не удалось найти место для уворота — пробуется уменьшение дистанции
  for (const dir of directions) {
    for (let dist = dodgeDistance * 0.6; dist >= 20; dist -= 10) {
      ...
      if (!checkWallCollision(testX, m.y, m.radius, true) && 
        ...
        const gridX = Math.floor(testX / CONFIG.cellSize);
        const gridY = Math.floor(testY / CONFIG.cellSize);
        if (gridX < 0 || gridX >= CONFIG.cols || gridY < 0 || gridY >= CONFIG.rows) continue;
  ...
  for (const m of state.monsters) {
    if (m.dodgeAnimation && m.dodgeAnimation.active) {
      ...
      if (anim.currentStep < anim.maxSteps) {
        ...
        // Анимация завершена
        anim.active = false;
        // Корректировка финальной позиции
        m.x = anim.endX;
        m.y = anim.endY;
...
export function handleMonsterTrapInteraction(m, i) {
  ...
  if (m.justSpawned) {
    if (m.justSpawnedTimer > 0) {
      m.justSpawnedTimer--;
      if (m.justSpawnedTimer <= 0) m.justSpawned = false;
    }
    return false;
  }

  // Пропускается, если монстр уже в анимации уворота
  if (m.dodgeAnimation && m.dodgeAnimation.active) return false;

  for (let t of state.traps) {
    // Проверка состояния ловушки
    ...
    if (Math.hypot(m.x - t.x, m.y - t.y) < m.radius + 12) {
      ...
      if (Math.random() > triggerChance) {
        ...
        if (dodged) {
          ...
          // Не удалось увернуться — применяется эффект
      ...
      // Монстр не пытается увернуться — применяется эффект
      // Ловушка считается сработавшей
      t.hasDealtDamage = true;
...
function applyTrapEffect(m, i, t) {
  ...
  switch (t.type) {
    ...
    case 'lightning': {
      ...
      // Созданине отдельного звука для каждого монстра
